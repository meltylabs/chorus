...
export class ProviderGroq implements IProvider {
    async streamResponse({
        ...
        const { canProceed, reason } = canProceedWithProvider("groq", apiKeys);
        if (!canProceed) {
            throw new Error(
                reason || "Please add your Groq API key in Settings.",
            );
        }
